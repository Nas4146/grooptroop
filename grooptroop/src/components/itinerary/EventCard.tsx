...
export default function EventCard({ event, isSelected = false }: EventCardProps) {
  ...
  return (
    <TouchableOpacity
      onPress={handlePress}
      className={`mb-4 rounded-2xl px-4 py-3 shadow-sm ${
        isSelected 
      ...
      <View className="flex-row justify-between items-start">
        <View className="flex-1">
          <View className="flex-row items-center">
            ...
            {event.isOptional && (
              <View className="ml-2 px-2 py-0.5 bg-gray-200 rounded-full">
                <Text className="text-xs text-gray-600">Optional</Text>
              </View>
            )}
          </View>
          ...
          <View className="flex-row items-center mt-1">
            ...
            <Text className="text-gray-500 text-sm ml-1">{event.time}</Text>
